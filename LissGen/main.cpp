...
#define BUFFER_SIZE 2048 // Aumentei um pouco para um rastro mais longo se precisar
...
// createShaderProgram, getStep (sem alterações)
GLuint createShaderProgram() {
    ...
    if (!success) { glGetShaderInfoLog(vertexShader, 512, NULL, infoLog); std::cerr << "ERRO::SHADER::VERTEX::COMPILATION_FAILED\n" << infoLog << std::endl; }
    GLuint fragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(fragmentShader, 1, &fragmentShaderSource, NULL);
    glCompileShader(fragmentShader);
    glGetShaderiv(fragmentShader, GL_COMPILE_STATUS, &success);
    if (!success) { glGetShaderInfoLog(fragmentShader, 512, NULL, infoLog); std::cerr << "ERRO::SHADER::FRAGMENT::COMPILATION_FAILED\n" << infoLog << std::endl; }
    GLuint shaderProgram = glCreateProgram();
    glAttachShader(shaderProgram, vertexShader);
    glAttachShader(shaderProgram, fragmentShader);
    glLinkProgram(shaderProgram);
    glGetProgramiv(shaderProgram, GL_LINK_STATUS, &success);
    if (!success) { glGetProgramInfoLog(shaderProgram, 512, NULL, infoLog); std::cerr << "ERRO::SHADER::PROGRAM::LINKING_FAILED\n" << infoLog << std::endl; }
...
int audioCallback(const void* inputBuffer, void* outputBuffer,
    ...
    for (unsigned long i = 0; i < framesPerBuffer; i++) {
        double t = state->phaseAccumulator;
        float sumL = 0.0f, sumR = 0.0f;
        int countL = 0, countR = 0;
        for (auto& row : state->channelL) if (!row.muted) { sumL += std::sin(2.0 * PI * row.freq * t); countL++; }
        for (auto& row : state->channelR) if (!row.muted) { sumR += std::sin(2.0 * PI * row.freq * t); countR++; }
        float sampleL = countL > 0 ? sumL / countL : 0.0f;
        float sampleR = countR > 0 ? sumR / countR : 0.0f;
        *out++ = sampleL * 0.3f;
        *out++ = sampleR * 0.3f;
        state->phaseAccumulator += 1.0 / SAMPLE_RATE;
        if (i % 2 == 0) {
            std::lock_guard<std::mutex> lock(state->bufferMutex);
            state->lissajousL.push_back(sampleL);
            state->lissajousR.push_back(sampleR);
            if (state->lissajousL.size() > BUFFER_SIZE) {
                state->lissajousL.erase(state->lissajousL.begin());
                state->lissajousR.erase(state->lissajousR.begin());
            }
        }
    }
    return paContinue;
...
// <<< --- FUNÇÃO DE DESENHO MODIFICADA PARA OS NOVOS MODOS --- >>>
void drawLissajousGL(AudioState& state, int x, int y, int width, int height, GLuint shaderProgram, GLuint vao, GLuint vbo) {
    ...
    {
        std::lock_guard<std::mutex> lock(state.bufferMutex);
        dataL = state.lissajousL;
        dataR = state.lissajousR;
    }
    ...
    float scale = (std::min)(width, height) / 2.0f - 20.0f;

    // Desenho do Grid e Círculos (sempre visíveis)
    glDisableVertexAttribArray(1);
    glVertexAttrib4f(1, 0.15f, 0.15f, 0.15f, 1.0f);
    glLineWidth(1.0f);
    ...
    for (float r = scale / 4.0f; r <= scale; r += scale / 4.0f) {
        ...
        for (int angle = 0; angle <= 360; angle += 5) {
            ...
            circleVertices.push_back(centerX + r * std::cos(rad));
            circleVertices.push_back(centerY + r * std::sin(rad));
        }
        glBufferData(GL_ARRAY_BUFFER, circleVertices.size() * sizeof(float), circleVertices.data(), GL_DYNAMIC_DRAW);
        glDrawArrays(GL_LINE_STRIP, 0, circleVertices.size() / 2);
    }
    ...
    for (size_t i = 0; i < dataL.size(); i++) {
        maxVal = (std::max)(maxVal, std::abs(dataL[i]));
        maxVal = (std::max)(maxVal, std::abs(dataR[i]));
    }

    if (state.laserMode) {
        // --- MODO LASER: Apenas o ponto final ---
        glDisableVertexAttribArray(1);
        if (dataL.size() > 0) {
            float endX = centerX + (dataL.back() / maxVal) * scale;
            float endY = centerY - (dataR.back() / maxVal) * scale;
            float endPoint[] = { endX, endY };

            // Efeito de "glow"
            glPointSize(14.0f);
            glVertexAttrib4f(1, 0.5f, 1.0f, 0.5f, 0.5f); // Verde, 50% transparente
            glBufferData(GL_ARRAY_BUFFER, sizeof(endPoint), endPoint, GL_DYNAMIC_DRAW);
            glDrawArrays(GL_POINTS, 0, 1);

            // Ponto principal
            glPointSize(8.0f);
            glVertexAttrib4f(1, 1.0f, 1.0f, 1.0f, 1.0f); // Branco sólido
            glBufferData(GL_ARRAY_BUFFER, sizeof(endPoint), endPoint, GL_DYNAMIC_DRAW);
            glDrawArrays(GL_POINTS, 0, 1);
        }
    }
        ...
        size_t numPoints = (size_t)(dataL.size() * state.trailPercent / 100.0f);
        if (numPoints < 2) numPoints = 2;
        size_t start = dataL.size() - numPoints;

        std::vector<float> lissajousVertices;
        lissajousVertices.reserve(numPoints * 6);
        for (size_t i = start; i < dataL.size(); i++) {
            lissajousVertices.push_back(centerX + (dataL[i] / maxVal) * scale);
            lissajousVertices.push_back(centerY - (dataR[i] / maxVal) * scale);
            float progress = (float)(i - start) / (float)(numPoints - 1);
        ...
        glLineWidth(2.0f);
        glBufferData(GL_ARRAY_BUFFER, lissajousVertices.size() * sizeof(float), lissajousVertices.data(), GL_DYNAMIC_DRAW);
        glDrawArrays(GL_LINE_STRIP, 0, numPoints);

        glDisableVertexAttribArray(1);
        if (state.showStartEndPoints) {
            if (!lissajousVertices.empty()) {
                float startPoint[] = { lissajousVertices[0], lissajousVertices[1] };
                glPointSize(8.0f);
                glVertexAttrib4f(1, 1.0f, 0.0f, 0.0f, 1.0f);
                glBufferData(GL_ARRAY_BUFFER, sizeof(startPoint), startPoint, GL_DYNAMIC_DRAW);
                glDrawArrays(GL_POINTS, 0, 1);
            }
            if (lissajousVertices.size() >= 6) {
                float endPoint[] = { lissajousVertices[lissajousVertices.size() - 6], lissajousVertices[lissajousVertices.size() - 5] };
                glPointSize(10.0f);
...
int main(int argc, char* argv[]) {
    // ... (toda a inicialização de SDL, GL3W, ImGui, PortAudio é idêntica)
    ...
    while (!quit) {
        // ... (Loop de eventos SDL é idêntico)
        ...
        // ... (Controles de Play/Stop e Frequência são idênticos)
        if (state.running) { if (ImGui::Button("Stop", ImVec2(120, 40))) { Pa_StopStream(stream); state.running = false; } }
        else { if (ImGui::Button("Play", ImVec2(120, 40))) { Pa_StartStream(stream); state.running = true; } }
        ...
        if (ImGui::CollapsingHeader("Left Channel (X)", ImGuiTreeNodeFlags_DefaultOpen)) { for (size_t i = 0; i < state.channelL.size(); i++) { ImGui::PushID((int)i); ImGui::SetNextItemWidth(100); ImGui::InputFloat("Hz", &state.channelL[i].freq, 0.0f, 0.0f, "%.3f"); ImGui::SameLine(); if (ImGui::Button("+")) state.channelL[i].freq += getStep(state.shiftPressed, state.ctrlPressed); ImGui::SameLine(); if (ImGui::Button("-")) state.channelL[i].freq -= getStep(state.shiftPressed, state.ctrlPressed); ImGui::SameLine(); ImGui::Checkbox("M", &state.channelL[i].muted); ImGui::SameLine(); if (ImGui::Button("X")) { state.channelL.erase(state.channelL.begin() + i); ImGui::PopID(); break; } ImGui::PopID(); } if (ImGui::Button("+ Add Frequency##L")) { float lastFreq = state.channelL.empty() ? 440.0f : state.channelL.back().freq; state.channelL.push_back(FrequencyRow(lastFreq)); } }
        ImGui::Separator();
        if (ImGui::CollapsingHeader("Right Channel (Y)", ImGuiTreeNodeFlags_DefaultOpen)) { for (size_t i = 0; i < state.channelR.size(); i++) { ImGui::PushID(1000 + (int)i); ImGui::SetNextItemWidth(100); ImGui::InputFloat("Hz", &state.channelR[i].freq, 0.0f, 0.0f, "%.3f"); ImGui::SameLine(); if (ImGui::Button("+")) state.channelR[i].freq += getStep(state.shiftPressed, state.ctrlPressed); ImGui::SameLine(); if (ImGui::Button("-")) state.channelR[i].freq -= getStep(state.shiftPressed, state.ctrlPressed); ImGui::SameLine(); ImGui::Checkbox("M", &state.channelR[i].muted); ImGui::SameLine(); if (ImGui::Button("X")) { state.channelR.erase(state.channelR.begin() + i); ImGui::PopID(); break; } ImGui::PopID(); } if (ImGui::Button("+ Add Frequency##R")) { float lastFreq = state.channelR.empty() ? 440.0f : state.channelR.back().freq; state.channelR.push_back(FrequencyRow(lastFreq)); } }
        ImGui::Separator();
        ...
        ImGui::Checkbox("Laser Mode (Persistence of Vision)", &state.laserMode);
    ...
    // ... (toda a finalização de PortAudio, ImGui, SDL é idêntica)
    if (state.running) Pa_StopStream(stream);
